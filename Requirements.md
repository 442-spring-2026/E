...
When the user's daily screen time is greater than the configured limit:
...
When the Dashboard page is opened after the user completes onboarding:

D17. The page uses the child's saved profile information to determine the displayed screen time limit and values.  
